...
int PQCLEAN_NAMESPACE_prng_set(prng_t *ctx, const void *prng_seed, unsigned long prng_seedlen) {
    unsigned char seed[48];
    if ( prng_seedlen >= 48 ) {
        memcpy( seed, prng_seed, 48 );
    } else {
        memcpy( seed, prng_seed, prng_seedlen );
        PQCLEAN_NAMESPACE_hash_msg( seed + prng_seedlen, 48 - (unsigned)prng_seedlen, (const unsigned char *)prng_seed, prng_seedlen);
    }
...
int PQCLEAN_NAMESPACE_prng_gen(prng_t *ctx, unsigned char *out, unsigned long outlen) {
