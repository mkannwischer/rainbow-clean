...
static void calculate_t4(unsigned char *t2_to_t4, const unsigned char *t1, const unsigned char *t3) {
    ...
    for (unsigned int i = 0; i < _O2; i++) { /// t3 width
        ...
        PQCLEAN_NAMESPACE_gf256v_add(t4, temp, _V1_BYTE);
        t4 += _V1_BYTE;
        t3 += _O1_BYTE;
    }
}
...
static void obsfucate_l1_polys(unsigned char *l1_polys, const unsigned char *l2_polys, unsigned int n_terms, const unsigned char *s1) {
    ...
    while (n_terms--) {
        ...
        PQCLEAN_NAMESPACE_gf256v_add(l1_polys, temp, _O1_BYTE);
        l1_polys += _O1_BYTE;
        l2_polys += _O2_BYTE;
    }
}
...
static void generate_secretkey(sk_t *sk, const unsigned char *sk_seed) {
    ...
    // set up prng
    prng_t prng0;
    PQCLEAN_NAMESPACE_prng_set(&prng0, sk_seed, LEN_SKSEED);
...
void PQCLEAN_NAMESPACE_generate_keypair(pk_t *rpk, sk_t *sk, const unsigned char *sk_seed) {
    ...
    PQCLEAN_NAMESPACE_calculate_Q_from_F(&pk, sk, sk); // compute the public key in ext_cpk_t format.
    ...
    PQCLEAN_NAMESPACE_extcpk_to_pk(rpk, &pk); // convert the public key from ext_cpk_t to pk_t.
}
#endif
...
void PQCLEAN_NAMESPACE_generate_keypair_cyclic(cpk_t *pk, sk_t *sk, const unsigned char *pk_seed, const unsigned char *sk_seed) {
    ...
    sk_t *Qs = &inst_Qs;
    prng_t *prng1 = &prng;
    PQCLEAN_NAMESPACE_prng_set(prng1, pk_seed, LEN_PKSEED);
...
void PQCLEAN_NAMESPACE_generate_compact_keypair_cyclic(cpk_t *pk, csk_t *rsk, const unsigned char *pk_seed, const unsigned char *sk_seed) {
    memcpy(rsk->pk_seed, pk_seed, LEN_PKSEED);
    memcpy(rsk->sk_seed, sk_seed, LEN_SKSEED);
    sk_t sk;
    PQCLEAN_NAMESPACE_generate_keypair_cyclic(pk, &sk, pk_seed, sk_seed);
}
#endif
...
void PQCLEAN_NAMESPACE_generate_secretkey_cyclic(sk_t *sk, const unsigned char *pk_seed, const unsigned char *sk_seed) {
    ...
    PQCLEAN_NAMESPACE_calculate_F_from_Q(sk, Qs, sk);
...
void PQCLEAN_NAMESPACE_cpk_to_pk(pk_t *rpk, const cpk_t *cpk) {
    ...
    // setup prng
    prng_t prng0;
    PQCLEAN_NAMESPACE_prng_set(&prng0, cpk->pk_seed, LEN_SKSEED);
    ...
    PQCLEAN_NAMESPACE_extcpk_to_pk(rpk, &pk);
}
#endif
