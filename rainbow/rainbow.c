...
int PQCLEAN_NAMESPACE_rainbow_sign(uint8_t *signature, const sk_t *sk, const uint8_t *_digest) {
    ...
    PQCLEAN_NAMESPACE_hash_msg(prng_seed, _HASH_LEN, prng_preseed, _HASH_LEN + LEN_SKSEED);
    PQCLEAN_NAMESPACE_prng_set(&prng_sign, prng_seed, _HASH_LEN); // seed = H( sk_seed || digest )
    ...
    while (!l1_succ) {
        ...
        PQCLEAN_NAMESPACE_prng_gen(&prng_sign, vinegar, _V1_BYTE);   // generating vinegars
    ...
    while (!succ) {
        ...
        PQCLEAN_NAMESPACE_prng_gen(&prng_sign, salt, _SALT_BYTE);                         // roll the salt
        PQCLEAN_NAMESPACE_hash_msg(_z, _PUB_M_BYTE, digest_salt, _HASH_LEN + _SALT_BYTE); // H(digest||salt)

        //  y = S^-1 * z
        memcpy(y, _z, _PUB_M_BYTE); // identity part of S
        gfmat_prod(temp_o, sk->s1, _O1_BYTE, _O2, _z + _O1_BYTE);
        PQCLEAN_NAMESPACE_gf256v_add(y, temp_o, _O1_BYTE);

        // Central Map:
        // layer 1: calculate x_o1
        memcpy(temp_o, r_l1_F1, _O1_BYTE);
        PQCLEAN_NAMESPACE_gf256v_add(temp_o, y, _O1_BYTE);
        gfmat_prod(x_o1, mat_l1, _O1_BYTE, _O1, temp_o);

        // layer 2: calculate x_o2
        PQCLEAN_NAMESPACE_gf256v_set_zero(temp_o, _O2_BYTE);
        gfmat_prod(temp_o, mat_l2_F2, _O2_BYTE, _O1, x_o1);             // F2
        batch_quad_trimat_eval(mat_l2, sk->l2_F5, x_o1, _O1, _O2_BYTE); // F5
        PQCLEAN_NAMESPACE_gf256v_add(temp_o, mat_l2, _O2_BYTE);
        PQCLEAN_NAMESPACE_gf256v_add(temp_o, r_l2_F1, _O2_BYTE); // F1
        PQCLEAN_NAMESPACE_gf256v_add(temp_o, y + _O1_BYTE, _O2_BYTE);

        // generate the linear equations of the 2nd layer
        gfmat_prod(mat_l2, sk->l2_F6, _O2 * _O2_BYTE, _O1, x_o1); // F6
        PQCLEAN_NAMESPACE_gf256v_add(mat_l2, mat_l2_F3, _O2 * _O2_BYTE);            // F3
    ...
    PQCLEAN_NAMESPACE_gf256v_add(signature, w, _PUB_N_BYTE);
    PQCLEAN_NAMESPACE_gf256v_add(signature + _PUB_N_BYTE, salt, _SALT_BYTE);
    return 0;
}
...
int PQCLEAN_NAMESPACE_rainbow_verify(const uint8_t *digest, const uint8_t *signature, const pk_t *pk) {
    ...
    PQCLEAN_NAMESPACE_hash_msg(correct, _PUB_M_BYTE, digest_salt, _HASH_LEN + _SALT_BYTE); // H( digest || salt )
...
///////////////  cyclic version  ///////////////////////////
int PQCLEAN_NAMESPACE_rainbow_sign_cyclic(uint8_t *signature, const csk_t *csk, const uint8_t *digest) {
    unsigned char sk[sizeof(sk_t) + 32];
    PQCLEAN_NAMESPACE_generate_secretkey_cyclic((sk_t *)sk, csk->pk_seed, csk->sk_seed); // generating classic secret key.
    return PQCLEAN_NAMESPACE_rainbow_sign(signature, (sk_t *)sk, digest);
}
#endif

#if defined(_RAINBOW_CYCLIC) || defined(_RAINBOW_CYCLIC_COMPRESSED)
int PQCLEAN_NAMESPACE_rainbow_verify_cyclic(const uint8_t *digest, const uint8_t *signature, const cpk_t *_pk) {
    unsigned char pk[sizeof(pk_t) + 32];
    PQCLEAN_NAMESPACE_cpk_to_pk((pk_t *)pk, _pk); // generating classic public key.
    return PQCLEAN_NAMESPACE_rainbow_verify(digest, signature, (pk_t *)pk);
}
#endif
